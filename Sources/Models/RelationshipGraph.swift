...
public class RelationshipGraph {
    ...
    private func consolidateExtensions(for typeName: String) {
        ...
        // Only remove relationships that will be regenerated by addTypeRelationships()
        // Preserve other relationships like protocol analysis, deep type analysis, etc.
        let relationshipsToRegenerate: Set<Relationship.Kind> = [
            .inheritance, .protocolConformance, .dependency, .composition, .aggregation
        ]
        relationships = relationships.filter { relationship in
            !(relationship.from == typeName && relationshipsToRegenerate.contains(relationship.kind))
        }
    ...
    private func mergeExtensionIntoType(baseType: TypeInfo, extension extensionType: TypeInfo) -> TypeInfo {
        ...
        // Mark extension methods as coming from extension
        let extensionMethods = extensionType.methods.map { method in
            MethodInfo(
                name: method.name,
                ...
                rethrows: method.rethrows,
                isMutating: method.isMutating,
                isNonMutating: method.isNonMutating,
        ...
        let combinedMethods = baseType.methods + extensionMethods
        
        // Mark extension properties as coming from extension
        let extensionProperties = extensionType.properties.map { property in
            PropertyInfo(
                name: property.name,
                typeName: property.typeName,
                accessLevel: property.accessLevel,
                isStatic: property.isStatic,
                isLet: property.isLet,
                isComputed: property.isComputed,
                isLazy: property.isLazy,
                isWeak: property.isWeak,
                isUnowned: property.isUnowned,
                hasGetter: property.hasGetter,
                hasSetter: property.hasSetter,
                getterAccessLevel: property.getterAccessLevel,
                setterAccessLevel: property.setterAccessLevel,
                hasWillSet: property.hasWillSet,
                hasDidSet: property.hasDidSet,
                defaultValue: property.defaultValue,
                attributes: property.attributes,
                isFromExtension: true
            )
        }
        let combinedProperties = baseType.properties + extensionProperties
        
        // Mark extension initializers as coming from extension
        let extensionInitializers = extensionType.initializers.map { initializer in
            InitializerInfo(
                parameters: initializer.parameters,
                accessLevel: initializer.accessLevel,
                isFailable: initializer.isFailable,
                isConvenience: initializer.isConvenience,
                isRequired: initializer.isRequired,
                throws: initializer.throws,
                isAsync: initializer.isAsync,
                genericParameters: initializer.genericParameters,
                genericConstraints: initializer.genericConstraints,
                whereClause: initializer.whereClause,
                attributes: initializer.attributes,
                isFromExtension: true
            )
        }
        let combinedInitializers = baseType.initializers + extensionInitializers
        
        // Combine other arrays (these don't need extension marking for now)
